...
func spawn_entity(data: Dictionary) -> Node2D:
	...
	match runtime_template:
		...
		"companion_palico":
			node = _make_palico(data, sidecar)
		"companion_rush":
			node = _make_rush_adapter(data, sidecar)
		"anvil_upgrade":
			node = _make_anvil(data, sidecar)
		"crafting_bench":
			node = _make_crafting_bench(data, sidecar)
		"bbq_spit":
			node = _make_bbq_spit(data, sidecar)
		"wall_run_surface":
			node = _make_wall_run_surface(data, sidecar)
		"ceiling_run_surface":
			node = _make_ceiling_run_surface(data, sidecar)
		"speed_booster_block":
			node = _make_speed_booster_block(data, sidecar)
		"grapple_ring":
			node = _make_grapple_ring(data, sidecar)
		"climbable_vine":
			node = _make_climbable_vine(data, sidecar)
		"climbable_rope":
			node = _make_climbable_rope(data, sidecar)
		"clear_pipe":
			node = _make_clear_pipe(data, sidecar)
		"logic_and":
			node = _make_logic_gate(data, sidecar, "and")
		"logic_or":
			node = _make_logic_gate(data, sidecar, "or")
		"logic_not":
			node = _make_logic_gate(data, sidecar, "not")
		_:
